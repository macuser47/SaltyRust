...
public class Wav$WavInputStream extends FilterInputStream {
   ...
   public Wav$WavInputStream(class_148 var1) {
      super(var1.method_1140());
         ...
         if (this.read() == 82 && this.read() == 73 && this.read() == 70 && this.read() == 70) {
            ...
            if (this.read() == 87 && this.read() == 65 && this.read() == 86 && this.read() == 69) {
               ...
               if (var3 != 1) {
                  ...
                  throw new class_145("WAV files must be PCM, unsupported format: " + var6 + " (" + var3 + ")");
               } else {
                  this.channels = this.read() & 255 | (this.read() & 255) << 8;
                  if (this.channels != 1 && this.channels != 2) {
                     throw new class_145("WAV files must have 1 or 2 channels: " + this.channels);
                  } else {
                     this.sampleRate = this.read() & 255 | (this.read() & 255) << 8 | (this.read() & 255) << 16 | (this.read() & 255) << 24;
                     this.skipFully(6);
                     int var4 = this.read() & 255 | (this.read() & 255) << 8;
                     if (var4 != 16) {
                        throw new class_145("WAV files must have 16 bits per sample: " + var4);
                     } else {
               ...
               throw new class_145("Invalid wave file header: " + var1);
            }
         } else {
            throw new class_145("RIFF header not found: " + var1);
         }
      } catch (Throwable var5) {
         class_165.method_1202(this);
         throw new class_145("Error reading WAV file: " + var1, var5);
      }
   }
